...
| **V2.20** | 2026-09-25 | Numéro ORIAS de Lucas (152032518M5276098) obtenu, remplace les 3 placeholders « à compléter » laissés en V2.19 : pied de page de `index.html`, `merci.html` et `confidentialite.html`. Ajouté aussi en `identifier` dans le JSON-LD `FinancialService` (`index.html`), sur le même principe que ce qui existait pour Capfinances avant son retrait. |
| **V2.21** | 2026-09-25 | Ajout de deux skills Claude Code dans `.claude/skills/` (chargés automatiquement par les sessions qui travaillent sur ce dépôt, jamais publiés en ligne : Jekyll ignore les dossiers commençant par un point) : **`emil-design-eng`** (principes de design et d'animation d'Emil Kowalski, repris de `emilkowalski/skills` au commit `d16ebe6`) et **`design-taste-frontend`** (« taste-skill », anti-design générique, repris de `Leonxlnx/taste-skill` au commit `c184364`). Les deux sont sous licence MIT, fichier LICENSE conservé à côté de chaque skill. Relus en entier avant ajout : uniquement des consignes de design, aucune dépendance à Figma ni à un outil externe. À savoir avant de s'en servir : le taste-skill vise des sites React/Tailwind (ce site est un fichier HTML unique) et classe comme « réflexes d'IA » plusieurs choix déjà en place ici (police Fraunces, palette crème/laiton, tirets longs « — » dans les textes). Il prévoit un mode « refonte en préservant l'identité existante » : ces éléments ne doivent pas être changés sans accord explicite de Lucas. |
